...
- [ ] A.1 Add `src/lib/global-registry.ts`: read `~/.iris/registry.json` and,
...
- [ ] A.2 Define the `GlobalDashboardModel` shape (project list with id, name,
...
- [ ] B.1 Add `src/templates/pages/global-dashboard.ts` rendering the
...
- [ ] B.2 Reuse the existing page shell (`src/templates/shell.ts`) and common
...
- [ ] C.1 Add `--global` to `iris open` in `src/commands/open.ts` and
...
- [ ] D.1 Wire the opportunistic global dashboard refresh into the final step
...
- [ ] D.2 Wire the global registry module into the render path so the
...
- [ ] E.1 Offline smoke: on a multi-project machine, `iris render --all`
...
- [ ] E.2 Run the full release gate and strict OpenSpec validation.

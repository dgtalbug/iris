...
- [x] A.1 Add `src/lib/terminal.ts`: color/box/prompt primitives honoring
...
- [x] A.2 Add `src/lib/host-adapters.ts`: data-driven adapter table for
...
- [x] A.3 Add curated prompt/color dependencies, pinned, each recorded in
      `package.json`'s dependency rationale. **Owns:** `package.json`

## WS-B. User-global state (`~/.iris`) (depends on nothing; owns its module)

- [x] B.1 Add `src/lib/user-config.ts`: read/write `~/.iris/config.json`,
...
- [x] B.2 Implement `migrateMetaToHome(cwd)`: atomic move of `iris/state.json`,
...
- [x] C.1 Add `src/lib/markdown-electric.ts`: hand-rolled container/fence
...
- [x] C.2 Wire section-id mapping (ten fixed headings → `#tldr`…`#appendix`),
      auto TOC, and meta-row into the markdown pipeline. **Owns:** `src/lib/markdown.ts`
- [x] C.3 Add `templates/research/blueprint.md` research scaffold.
      **Owns:** `templates/research/blueprint.md`

## WS-D. Contract schemas + page templates (owns schemas + page templates)

- [x] D.1 Add optional additive `sections.blueprint` object to the bug, feature,
...
- [x] D.2 Compose blueprint narrative sections above existing typed widgets when
...
- [x] E.1 Add `src/lib/provenance.ts` (denylist scanner) and
...
- [x] E.2 Rename the design language to Iris Electric across user-facing copy and
...
- [x] F.1 Restructure `templates/agents/iris-workspace/` as a directory:
...
- [x] F.2 Generate `references/components.md` from the same canonical source as
...
- [x] F.3 Add `templates/agents/iris-guard/SKILL.md` (authoring-time provenance
      rules + embedded denylist). **Owns:** `templates/agents/iris-guard/**`
- [x] F.4 Make `src/lib/agent-skills.ts` adapter-driven and emit skill
...
- [x] G.1 Rework `src/commands/init.ts` into the staged flow (welcome → validate
...
- [x] G.2 Add the new init flags and implement the `config` verb in `src/cli.ts`.
      **Owns:** `src/cli.ts`
- [x] G.3 Resolve page registry and spec snapshot via `~/.iris/projects/<id>/` in
...
- [x] G.4 Wire the provenance scan into init/update/render and `release:check`.
      **Owns:** `package.json` (release:check script), `src/commands/init.ts` (scan call)

## WS-H. Verification (depends on G)

- [x] H.1 Offline smoke: `iris init --yes --tools none` runs with no network and
      no prompt; `scripts/install-smoke.mjs` passes. **Owns:** `scripts/install-smoke.mjs`
- [x] H.2 Migration, denylist, and container render snapshot tests pass.
- [x] H.3 Run the full release gate and strict OpenSpec validation.

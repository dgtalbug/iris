...
- [x] A.1 Add `src/lib/indexing.ts`: indexer discovery (`PATH` then
...
- [x] A.2 Implement staleness computation: compare `lastIndexedSha` to the
...
- [x] B.1 Add an Index card to the Overview page template that reads
...
- [x] B.2 Add the same Index card to the Commands page template.
...
- [x] B.3 Add render-snapshot tests for the Index card in both enabled and
      disabled states. **Owns:** `tests/pages-index-card.test.ts`

## WS-C. Agent skill: index-aware section (owns skill template)

- [x] C.1 Add a "When the index is enabled" section to the `iris-workspace`
...
- [x] C.2 Regenerate the skill's component reference if the section touches
...
- [x] C.3 Add a test that the index-aware section renders for every host that
...
- [x] D.1 Add an `index.json` shape description to the user-global state
...
- [x] D.2 Confirm `iris/config.yaml` records nothing about indexing: add a
...
- [x] E.1 Wire `iris init --index` to call `src/lib/indexing.ts`: discover the
...
- [ ] E.2 If the existing `--index` / `--no-index` plumbing in `src/cli.ts`
      ...
      (only if needed)
- [x] E.3 Wire the Index card read path into the render pipeline so the

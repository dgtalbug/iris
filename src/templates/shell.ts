  ...
  const docs = options.projectDocs
    .map((name) => {
      ...
      return navItem({
        ...
        label: meta.label,
        iconName: meta.icon as IconName,
  ...
  return `<div class="work-drawer-shell" data-work-drawer hidden>
      ...
      <aside class="work-drawer" role="dialog" aria-modal="true" aria-labelledby="work-drawer-title" aria-describedby="work-drawer-description" tabindex="-1">
        <header class="work-drawer-header">
          <div><span class="badge b-muted" data-work-drawer-type>type</span> <span class="mono" data-work-drawer-id>id</span></div>
          <button class="btn btn-outline" type="button" data-work-drawer-close>Close</button>
        </header>
        <div class="work-drawer-body">
          ...
          <div class="work-drawer-actions"><a class="btn btn-primary" data-work-drawer-full-page href="./index.html">Open full page</a></div>
        </div>
      </aside>
    </div>`;
}
    ...
    <div class="app" data-dashboard-main>
      ...
      <div class="main">
        <header class="topbar" data-iris-nav>
          <button class="btn btn-ghost menu-button" type="button" data-menu-toggle aria-expanded="false" aria-label="Show workspace sections">${icon('chrome-menu')}</button>
          <nav class="crumbs" aria-label="Breadcrumb">${renderCrumbs(options.crumbs)}</nav>
          <span class="topbar-spacer"></span>
          ${options.topbar ?? ''}
          <div class="mode-toggle" data-theme-toggle-group>
            <button type="button" data-theme-set="dark" aria-pressed="false">${icon('chrome-dark')} Dark</button>
            <button type="button" data-theme-set="light" aria-pressed="false">${icon('chrome-light')} Light</button>
          </div>
        </header>
        <main class="content">
${options.content}

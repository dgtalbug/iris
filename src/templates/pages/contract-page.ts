...
function renderTimeline(events: unknown[]): string {
  ...
  const items = events
    .map((event) => {
      ...
      const tone =
        { warn: ' class="warn"', warning: ' class="warn"', error: ' class="danger"', danger: ' class="danger"' }[
          level
        ] ?? '';
      return `<li${tone}>
...
export function projectPlaceholderHtml(
  ...
  const content = `<div class="page-head">
      ...
      <div class="page-head-actions"><span class="badge b-muted">not written yet</span></div>
    </div>
    ...
    ${
      siblings.length === 0
        ? ''
        : `<section class="card project-strip">

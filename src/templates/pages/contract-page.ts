...
import { apertureGlyph, escapeHtml, statusChip, typeChip, PROJECT_DOC_NAMES } from '../common.js';
...
export function projectPlaceholderHtml(
  ...
  const content = `<div class="page-head">
    ...
    <article class="surface empty-state">
      ...
      <p>Create intentional content with the installed Iris skill and content commands, then refresh with <code>iris render --all</code>.</p>
    </article>`;
  ...
  return renderShell({
    ...
    counts: context.counts,
    projectDocs: context.projectDocs,
    crumbs: [{ label: 'iris', href: '../index.html' }, { label: 'project docs' }, { label: name }],
    content,

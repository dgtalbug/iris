...
## What iris Is / Is Not
iris is an open-source local skill + CLI that renders contract data into deterministic HTML.

Non-goals (v1): no SaaS, no server, no auth, no telemetry, no MCP integration, no analysis engine, no lifecycle engine.
...
## v1 Scope
M0 foundation + M1 design system and dashboard, then typed templates and distribution milestones.

...
> Snapshot date: 2026-08-21 · branch `feat/render-openspec-markdown` · semantic OpenSpec Markdown implementation verified and archived before hosted PR checks.
...
| Runtime dependencies | `ajv` plus generation-time `markdown-it`; no frontend framework, browser Markdown runtime, server, watcher, telemetry, or OpenSpec runtime dependency |
...
- Markdown artifacts render semantically with embedded HTML, unsafe destinations, and active images disabled; exact escaped source remains available and YAML stays literal.
- The parser reports filesystem evidence, not the result of `openspec validate`.

## Remaining risks and gates

1. Semantic Markdown dogfood, local gates, OpenSpec verification, spec sync, and archive are complete; hosted PR checks and merge remain pending.
